...
class AtariDQNAgent(DQNBaseAgent):
	...
	def update_behavior_network(self):
		...
		with torch.no_grad():
			max_action = self.behavior_net(next_state).max(1)[1].view(-1, 1)   	### different from DQN
			q_next = self.target_net(next_state).gather(1, max_action)			### different from DQN
			#q_next = self.target_net(next_state).max(1)[0].view(-1, 1)			### DQN
