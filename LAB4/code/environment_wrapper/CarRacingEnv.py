...
class CarRacingEnvironment:
	...
	def under_road(self, obs):
		# cut the image to get the part where the car is
		part_image = obs[67:77, 45:51, :]
	...
	def step(self, action):
		...
		#print("road_pixel_count - ",road_pixel_count)
		#print("grass_pixel_count - ",grass_pixel_count)
		#print("if in the road - ",self.under_road(obs))
		#print(" ")

		# my reward shaping strategy, you can try your own
		if not self.under_road(obs):
			#reward += -(grass_pixel_count)/300
			if grass_pixel_count < 190:
				reward += -0.2
			elif grass_pixel_count < 200:
				reward += -0.4
			elif grass_pixel_count < 220:
				reward += -1.0
			elif grass_pixel_count < 240:
				reward += -1.4
			elif grass_pixel_count < 260:
				reward += -1.9
			else:
				reward += -2.5
