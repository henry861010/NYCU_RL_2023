...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for( int i{0} ; i < iso_last ; i++){   //each isomorphic
			est = est +  weight[ indexof(isomorphic[i],b) ];
		}
		return est;
	}
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO
		size_t index{0};
		for( int j{0} ; j < patt.size() ; j++){  //each title of the isomorphic
			index = index<<4 | (size_t)b.at(patt[j]);
		}
		return index;
	}
...
public:
	...
	bool assign(const board& b) {
		...
		after = before = b;   //create new item
			...
			if (move->assign(b)) {
				...
				for(size_t i{0};i<16;i++){  
					if(move->after_state().at(i)==0){  //find the empty tile in the board 
						...
						total += estimate(boardAdd2)*0.9 + estimate(boardAdd4)*0.1;  //the probability if the 2 is 0.9 and 4 is 0.1
					}
				}
//std::cout<<"action-"<<move->action()<<"action reward: "<<move->reward()<<"  value:"<<move->reward() + total/emptyNumber <<std::endl;
				move->set_value( move->reward() + total/emptyNumber );  //do not need to check if the emptyNumber is 0. if it is 0, reprsents the game is end!(not enter "move->assign(b)") 
...
/*
std::cout<<"the best action is "<<best->action()<<std::endl;
board temp0 = best->before_state();
board temp1 = best->after_state();
temp0.show();
std::cout<<std::endl;		
temp1.show();
std::cout<<"======================"<<std::endl;
